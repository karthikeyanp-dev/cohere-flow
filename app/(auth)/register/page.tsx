...
export default function RegisterPage() {
  ...
  return (
    <div className="glass rounded-2xl p-8 animate-fade-in">
      ...
      <p className="text-sm mb-6" style={{ color: 'var(--text-secondary)' }}>Join your team workspace</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {[
          { label: 'Full Name', type: 'text', val: name, set: setName, ph: 'Jane Smith' },
        ...
        <button type="submit" disabled={loading}
          ...
          style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: 'white', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }}>
      ...
      <button onClick={handleGoogle} disabled={googleLoading}
        ...
        style={{ background: 'var(--bg-raised)', border: '1px solid var(--border)', color: 'var(--text-primary)', cursor: googleLoading ? 'not-allowed' : 'pointer' }}>
      ...
      <p className="text-center text-sm mt-6" style={{ color: 'var(--text-secondary)' }}>
        ...
        <Link href="/login" className="font-medium hover:underline" style={{ color: 'var(--brand)' }}>Sign in</Link>
      </p>
    </div>
  );
}
